...
export default function Dashboard() {
  ...
  return (
    <div>
      ...
      {/* ═══════════ ANALYTICS ═══════════ */}
      {tab === 'analytics' && (
        <div className={`tab-content enter-${tabDirection}`} key="analytics">
          <div className="section-label">Executive Forecasting & Analytics</div>
          ...
          {mlForecasts.length > 0 ? (() => {
            ...
            mlForecasts.forEach(row => {
              if (!grouped[row.ProductName]) {
                grouped[row.ProductName] = { 
                  name: row.ProductName, 
                  safetyStock: row.SafetyStock, 
                  rop: row.ROP, 
                  orderQty: row.RecOrderQty 
                };
                totalSafetyStock += row.SafetyStock;
                totalRop += row.ROP;
                if (row.ROP > row.SafetyStock) highRiskCount++;
                skuCount++;
              }
              grouped[row.ProductName][`h${row.Horizon}`] = row.ForecastTotal;
            });
            const chartData = Object.values(grouped);
            
            const avgSafetyStock = Math.round(totalSafetyStock / skuCount).toLocaleString();
            const avgRop = Math.round(totalRop / skuCount).toLocaleString();
            ...
            return (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
                  <div className="intel-card" style={{ padding: '20px', borderTop: '3px solid var(--accent-blue)' }}>
                    <div style={{ color: 'var(--text-muted)', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Total SKUs Analyzed</div>
                    <div style={{ fontSize: '28px', fontWeight: 'bold', color: '#fff', marginTop: '8px' }}>{skuCount}</div>
                  </div>
                  <div className="intel-card" style={{ padding: '20px', borderTop: '3px solid var(--accent-emerald)' }}>
                    <div style={{ color: 'var(--text-muted)', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Avg Safety Stock</div>
                    <div style={{ fontSize: '28px', fontWeight: 'bold', color: '#fff', marginTop: '8px' }}>{avgSafetyStock}</div>
                  </div>
                  <div className="intel-card" style={{ padding: '20px', borderTop: '3px solid var(--accent-amber)' }}>
                    <div style={{ color: 'var(--text-muted)', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Avg Reorder Point</div>
                    <div style={{ fontSize: '28px', fontWeight: 'bold', color: '#fff', marginTop: '8px' }}>{avgRop}</div>
                  </div>
                </div>

                {/* Charts Row */}
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                  <div className="intel-card" style={{ height: '380px', padding: '24px' }}>
                    <div style={{ marginBottom: '4px', fontSize: '16px', fontWeight: 'bold', color: '#fff' }}>Forecast Trajectory Analysis</div>
                    <div style={{ marginBottom: '20px', fontSize: '12px', color: 'var(--text-muted)' }}>Projected demand volumes across tactical (7D), operational (30D), and strategic (90D) horizons.</div>
                    <ResponsiveContainer width="100%" height="85%">
                      <BarChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                        ...
                        <YAxis stroke="var(--text-muted)" fontSize={11} axisLine={false} tickLine={false} tickFormatter={(val) => val >= 1000 ? (val/1000).toFixed(1)+'k' : val} />
                        <Tooltip 
                          cursor={{ fill: 'rgba(255,255,255,0.02)' }}
                          contentStyle={{ background: 'rgba(15, 23, 42, 0.95)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.5)' }}
                          itemStyle={{ fontSize: '12px', fontWeight: '500' }}
                          labelStyle={{ color: '#94a3b8', marginBottom: '8px', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.5px' }}
                        />
                        <Legend wrapperStyle={{ fontSize: '12px', paddingTop: '10px' }} iconType="circle" />
                        <Bar dataKey="h7" name="7D Projection" fill="var(--accent-emerald)" radius={[4, 4, 0, 0]} />
                        <Bar dataKey="h30" name="30D Projection" fill="var(--accent-cyan)" radius={[4, 4, 0, 0]} />
                        <Bar dataKey="h90" name="90D Projection" fill="var(--accent-violet)" radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>

                  <div className="intel-card" style={{ height: '380px', padding: '24px' }}>
                    <div style={{ marginBottom: '4px', fontSize: '16px', fontWeight: 'bold', color: '#fff' }}>Inventory Risk Profile</div>
                    <div style={{ marginBottom: '20px', fontSize: '12px', color: 'var(--text-muted)' }}>Comparison of Safety Stock buffer versus ML-optimized Reorder Point (ROP).</div>
                    <ResponsiveContainer width="100%" height="85%">
                      <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                        ...
                        />
                        <Legend wrapperStyle={{ fontSize: '12px', paddingTop: '10px' }} iconType="circle" />
                        <Bar dataKey="safetyStock" name="Safety Stock Buffer" fill="var(--accent-amber)" radius={[4, 4, 0, 0]} barSize={30} />
                        <Line type="monotone" dataKey="rop" name="Target Reorder Point" stroke="var(--accent-rose)" strokeWidth={3} dot={{ r: 5, fill: 'var(--accent-rose)', strokeWidth: 2, stroke: '#0f172a' }} activeDot={{ r: 7 }} />
                      </ComposedChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                ...
                {/* Data Grid */}
                <div className="intel-card" style={{ padding: '24px' }}>
                  <div style={{ marginBottom: '16px', fontSize: '16px', fontWeight: 'bold', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span>Raw Dataset Extraction</span>
                    <span style={{ fontSize: '11px', fontWeight: 'normal', color: 'var(--text-muted)', background: 'rgba(255,255,255,0.05)', padding: '4px 8px', borderRadius: '4px' }}>{mlForecasts.length} Records</span>
                  </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                      <thead>
                        <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)', color: 'var(--text-muted)', textAlign: 'left' }}>
                          ...
                          <th style={{ padding: '12px', fontWeight: '500' }}>Model</th>
                          <th style={{ padding: '12px', fontWeight: '500' }}>Horizon</th>
                          <th style={{ padding: '12px', fontWeight: '500', textAlign: 'right' }}>Forecast</th>
                          <th style={{ padding: '12px', fontWeight: '500', textAlign: 'right' }}>Safety Stock</th>
                          <th style={{ padding: '12px', fontWeight: '500', textAlign: 'right' }}>ROP</th>
                          <th style={{ padding: '12px', fontWeight: '500', textAlign: 'right' }}>Order Qty</th>
                        </tr>
                      </thead>
                      <tbody>
                        {mlForecasts.map((row, idx) => (
                          <tr key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.02)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                            <td style={{ padding: '12px', color: 'var(--accent-blue)', fontFamily: 'monospace' }}>{row.SKU}</td>
                            <td style={{ padding: '12px', color: '#fff' }}>{row.ProductName}</td>
                            <td style={{ padding: '12px' }}><span style={{ background: 'rgba(255,255,255,0.05)', padding: '2px 6px', borderRadius: '4px', fontSize: '11px' }}>{row.BestModel}</span></td>
                            <td style={{ padding: '12px', color: 'var(--accent-emerald)' }}>{row.Horizon} Days</td>
                            <td style={{ padding: '12px', textAlign: 'right', fontWeight: '500' }}>{Number(row.ForecastTotal).toLocaleString()}</td>
                            <td style={{ padding: '12px', textAlign: 'right', color: 'var(--text-muted)' }}>{Number(row.SafetyStock).toLocaleString()}</td>
                            <td style={{ padding: '12px', textAlign: 'right', color: row.ROP > row.SafetyStock ? 'var(--accent-rose)' : 'var(--text-muted)' }}>{Number(row.ROP).toLocaleString()}</td>
                            <td style={{ padding: '12px', textAlign: 'right', color: '#fff' }}>{Number(row.RecOrderQty).toLocaleString()}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            <div style={{ padding: '60px 40px', textAlign: 'center', color: 'var(--text-muted)', background: 'rgba(255,255,255,0.02)', borderRadius: '12px', border: '1px dashed rgba(255,255,255,0.1)' }}>
              <Sparkles size={32} style={{ margin: '0 auto 16px auto', color: 'var(--accent-blue)', opacity: 0.5 }} />
              <div style={{ fontSize: '16px', color: '#fff', marginBottom: '8px' }}>No ML Forecast Data Found</div>
              <div>Please upload a valid CSV dataset in the Settings panel to generate analytical insights.</div>
            </div>
          )}
        </div>
      )}
